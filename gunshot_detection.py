...
def convert_audio_to_spectrogram(data, target_shape=None):
    ...
    # Ensure it's in float32 format
    spectrogram = spectrogram.astype(np.float32)
    
    # Handle audio of any length by resizing or padding the spectrogram
    if target_shape is not None:
        spectrogram = resize_or_pad_spectrogram(spectrogram, target_shape)
    
    return spectrogram

def resize_or_pad_spectrogram(spectrogram, target_shape):
    """
    Resizes or pads/trims the spectrogram to match the target shape.
    target_shape: Tuple (number_of_mels, target_width)
    """
    current_shape = spectrogram.shape
    
    # Pad or trim the number of mels (height)
    if current_shape[0] < target_shape[1]:
        print("edit1")
        pad_height = target_shape[1] - current_shape[0]
        spectrogram = np.pad(spectrogram, ((0, pad_height), (0, 0)), mode='constant')
    elif current_shape[0] > target_shape[1]:
        spectrogram = spectrogram[:target_shape[1],:]
    
    # Pad or trim the width (time steps)
    if current_shape[1] < target_shape[2]:
        print("edit2")
        pad_width = target_shape[2] - current_shape[1]
        spectrogram = np.pad(spectrogram, ((0, 0), (0, pad_width)), mode='constant')
    elif current_shape[1] > target_shape[2]:
        spectrogram = spectrogram[ :, :target_shape[2]]
...
def analyze_wav_file(file_path):
    ...
    microphone_data, _ = librosa.load(file_path, sr=AUDIO_RATE)

    # Post-process the microphone data
    ...
    # Reshape and prepare the data for model inference
    processed_data_1 = modified_microphone_data.reshape(input_shape_1)

    HOP_LENGTH = 345 * 2
    processed_data_2 = convert_audio_to_spectrogram(data=modified_microphone_data,target_shape=input_shape_2).reshape(input_shape_2)

    HOP_LENGTH = 345
    processed_data_3 = convert_audio_to_spectrogram(data=modified_microphone_data,target_shape=input_shape_3).reshape(input_shape_3)

    # Perform inference using the TensorFlow Lite models
    ...
    print(f"Model 1 probability: {probabilities_1[0]}")
    print(f"Model 2 probability: {probabilities_2[0]}")
    print(f"Model 3 probability: {probabilities_3[0]}")
    ...
    if model_1_activated or model_2_activated or model_3_activated:
...
# Example usage: analyze a given .WAV file
#wav_file_path = "Individual_Audio_Training_Sample_Batches/GAN_Revolvers/1.wav"
# wav_file_path = "Individual_Audio_Training_Sample_Batches/Microphone Samples/Single-Tap Samples with MINI Microphone/Gunshot Sound Sample #3.wav"
# analyze_wav_file(wav_file_path)
